...
// a rename — because that is what a file files is once it stops being a widget.
//
// The subject is plugins/files, built by plugins/stage.sh. plugins/edit is loaded beside it,
...
tree_app :: proc(t: ^testing.T, name: string) -> (a: app.App, root: string, ok: bool) {
    a = plug_app(t, name, "plugins/files", "plugins/edit") or_return
    app.plug_init(&a)
    ...
    // kind, so the files IS what a directory opens as, in the lane alt+f already goes to.
...
a_row_is_ls_la_with_a_link_on_the_name :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-shape")
    if !ok {
        return
    }
    ...
    testing.expect_value(t, line_text(snap, 0), "")
    ...
    for want, line in ([?]string{"..", "sub", "top.txt"}) {
        testing.expect_value(t, name_of(&a, line+1), want)
    }
    testing.expect(t, strings.has_suffix(line_text(snap, 1), "../"), line_text(snap, 1))
    testing.expect(t, strings.has_prefix(line_text(snap, 3), "-rw-"), line_text(snap, 3))
    testing.expect(t, strings.has_suffix(line_text(snap, 2), "sub/"), line_text(snap, 2))
    ...
    up, has_up := view.field_text(&snap.text, d, 1, "path")
    ...
    path, has_path := view.field_text(&snap.text, d, 3, "path")
...
enter_is_a_row_and_the_chain_is_the_navigation :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-enter")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    testing.expect(t, strings.contains(rows, "double-click = exec :fs.enter <path> && :open <path>"),
                   rows)
    testing.expect(t, strings.contains(rows, "click = fs.snap"), rows)
    ...
    testing.expect_value(t, name_of(&a, 1), "..")
    testing.expect_value(t, name_of(&a, 2), "deep.txt")
    ...
    testing.expect_value(t, txt.text_line_count(after), 4)
    testing.expect_value(t, point(&a).head.line, 2)
    testing.expect_value(t, name_of(&a, 2), "sub")
}

// "A file listing navigates and OPENS." The second half of the row: over a file `fs.enter` does
...
enter_over_a_file_falls_through_to_the_editor :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-open")
    if !ok {
        return
    }
    defer close_plug_app(&a)
...
the_caret_lands_at_the_end_of_every_name :: proc(t: ^testing.T) {
    a, _, ok := tree_app(t, "oket-files-caret")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    app.handle_chord(&a, chord("DOWN"))
    testing.expect_value(t, point(&a).head.line, 3)
...
// The side arrows are MOTION, not the hierarchy: the files asks for neither, so they stay the
...
the_side_arrows_move_the_caret_in_the_row :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-sides")
    if !ok {
        return
    }
    defer close_plug_app(&a)

    go_row(&a, 2) // sub, with the caret at the end of its name
    was := point(&a).head
...
the_listing_draws_ls_la_through_the_kernels_renderer :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-draw")
    if !ok {
        return
    }
    defer close_plug_app(&a)

    app.cl_exec(&a, fmt.tprintf(":fs.toggle %s/sub", root)) // the subtree, opened in place
    app.surface_fit(&a, 70, 6)
    app.surface_draw(&a)
    ...
    for want, i in ([?]string{"../", "sub/", "  deep.txt", "top.txt"}) {
        testing.expect(t, strings.has_prefix(rows[i+1], "drwx") || strings.has_prefix(rows[i+1], "-rw-"),
                       rows[i+1])
        testing.expect(t, strings.has_suffix(rows[i+1], want), rows[i+1])
        testing.expect_value(t, len(rows[i+1]), PREFIX + len(want))
    }
...
hover_underlines_the_link_it_would_follow :: proc(t: ^testing.T) {
    a, _, ok := tree_app(t, "oket-files-hover")
    if !ok {
        return
    }
    defer close_plug_app(&a)
...
a_directory_row_cannot_fill_the_file_hole :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-hole")
    if !ok {
        return
    }
    defer close_plug_app(&a)
...
a_value_that_is_not_what_it_draws :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-value")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    defer txt.snapshot_release(snap)
    defer desc.release(d)
    name, hi, _ := desc.field_span(d, 3, "name")
    path_lo, path_hi, _ := desc.field_span(d, 3, "path")
    testing.expect_value(t, path_lo, name) // the same bytes...
    testing.expect_value(t, path_hi, hi)
    shown, _ := view.field_text(&snap.text, d, 3, "name")
    acted, _ := view.field_text(&snap.text, d, 3, "path")
...
// A files takes typing and names a DIRECTORY, and a journal replaying a listing into one
...
@(test)
a_files_is_not_journaled :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-journal")
    if !ok {
        return
    }
    defer close_plug_app(&a)
...
typing_renames_and_the_link_still_names_the_file :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-type")
    if !ok {
        return
    }
    defer close_plug_app(&a)

    go_row(&a, 3) // top.txt, caret at the end of the name
    for r in "_v2" {
        app.text_input(&a, r)
    }
    testing.expect_value(t, name_of(&a, 3), "top.txt_v2")
    ...
    // survive it.
    txt.doc_set_head(doc_of(&a), {3, 0}, false)
    app.point_sync(&a)
    app.text_input(&a, 'z')
    snap, d := reading(&a)
    defer txt.snapshot_release(snap)
    defer desc.release(d)
    testing.expect_value(t, name_of(&a, 3), "ztop.txt_v2")
    testing.expect(t, strings.has_prefix(line_text(snap, 3), "-rw-"), line_text(snap, 3))
    // The link rode the splice: its span grew with the name, its value is still the file.
    path, _ := view.field_text(&snap.text, d, 3, "path")
    testing.expect_value(t, path, fmt.tprintf("%s/top.txt", root))
}
...
commit_renames_every_row_that_was_typed_over :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-commit")
    if !ok {
        return
    }
    defer close_plug_app(&a)
...
commit_refuses_a_name_that_is_taken :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-taken")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    app.cl_exec(&a, ":fs.reload")

    go_row(&a, 4) // top.txt, which sorts after taken.txt
    for _ in 0 ..< 20 {
        app.handle_chord(&a, chord("BKSP")) // stops at the head of the name, however long
    }
    testing.expect_value(t, name_of(&a, 4), "")
    for r in "taken.txt" {
        app.text_input(&a, r)
    }
...
backspace_stops_at_the_head_of_the_name :: proc(t: ^testing.T) {
    a, _, ok := tree_app(t, "oket-files-erase")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    testing.expect_value(t, name_of(&a, 3), "top.tx")
    ...
    defer txt.snapshot_release(snap)
    defer desc.release(d)
    testing.expect_value(t, txt.text_line_count(snap), 4) // no row was joined to the one above
    testing.expect_value(t, name_of(&a, 3), "")
    testing.expect(t, strings.has_prefix(line_text(snap, 3), "-rw-"), line_text(snap, 3))
    testing.expect_value(t, len(line_text(snap, 3)), PREFIX) // the prefix, and nothing after it
}
...
// lands on the first MATCH, so enter goes into it rather than up. The files has no head row,
...
the_filter_hides_rows_and_keeps_the_way_out :: proc(t: ^testing.T) {
    a, _, ok := tree_app(t, "oket-files-filter")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    }
    {
        ...
        defer txt.snapshot_release(snap)
        defer desc.release(d)
        testing.expect_value(t, line_text(snap, 0), "/top")
        testing.expect_value(t, txt.text_line_count(snap), 3) // filter + `..` and the match
    }
    testing.expect_value(t, name_of(&a, 1), "..")
    testing.expect_value(t, name_of(&a, 2), "top.txt") // the runes went to the filter
    testing.expect_value(t, point(&a).head.line, 2)
...
// carries the quotes. `fs.enter` is where that line ends and it wants the value: a directory
...
a_path_with_a_space_is_visited_like_any_other :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-space")
    if !ok {
        return
    }
    defer close_plug_app(&a)
    ...
    app.cl_exec(&a, ":fs.reload")
    testing.expect_value(t, txt.doc_line_count(doc_of(&a)), 5)

    go_row(&a, 3) // sub, "two words", top.txt — directories first
    app.handle_chord(&a, chord("RTRN"))
    ...
    testing.expect_value(t, name_of(&a, 2), "in.txt")
}
...
@(test)
a_directory_with_no_files_reports :: proc(t: ^testing.T) {
    a, ok := bare_app()
    if !ok {
        return
    }
...
the_way_out_is_not_a_name :: proc(t: ^testing.T) {
    a, root, ok := tree_app(t, "oket-files-dotdot")
    if !ok {
        return
    }
    defer close_plug_app(&a)
