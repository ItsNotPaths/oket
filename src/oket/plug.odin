...
Plugin :: struct {
    name:    string, // owned; its directory's name, and the section header in binds.conf
    path:    string, // owned
    lib:     dynlib.Library,
...
// net above — a plugin that dies on load must not make oket unstartable — and `--no-plugins`
// is the door for the case the net cannot hold.
plug_autoload :: proc(a: ^App) {
    ...
    for info in os.read_directory_iterator(&it) {
        // Directories only: an install overlays, so a loose `.so` beside one would load twice.
        if info.type != .Directory {
            continue
        }
        so := plug_path(a, info.name) // one place spells the layout
        if os.exists(so) {
            append(&found, so)
        }
    }
...
// A plugin is a DIRECTORY and its `.so` repeats the name (§7): a grammar or a script ships
// beside the library, and an uninstall is a delete of one path with no manifest to consult.
plug_path :: proc(a: ^App, name: string) -> string {
    file := fmt.tprintf("%s.so", name)
    path, _ := filepath.join({a.home.data, PLUGIN_DIR, name, file}, context.temp_allocator)
    return path
}
